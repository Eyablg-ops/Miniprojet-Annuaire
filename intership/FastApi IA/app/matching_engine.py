...
class EnhancedMatchingEngine:
    ...
    def extract_skills_from_offer(self, offer) -> List[str]:
        """Extract required skills from offer"""
        skills = []
        if hasattr(offer, 'requiredSkills') and offer.requiredSkills:
            skill_items = re.split(r'[,;\n]', offer.requiredSkills.lower())
            skills = [s.strip() for s in skill_items if s.strip()]
        return skills
    
    def calculate_skills_match(self, student_skills: List[str], offer_skills: List[str]) -> float:
        """Advanced skills matching with synonyms and partial matching"""
        if not student_skills or not offer_skills:
            return 0.0
        ...
        match_score = 0
        total_required = len(offer_skills)
        
        for required_skill in offer_skills:
            best_match = 0
    ...
    def calculate_education_match(self, student, offer) -> float:
        """Improved education matching with better level recognition"""
        # Safely get offer text
        offer_text = ""
        if hasattr(offer, 'title'):
            offer_text += offer.title or ""
        if hasattr(offer, 'description'):
            offer_text += " " + (offer.description or "")
        if hasattr(offer, 'requiredSkills'):
            offer_text += " " + (offer.requiredSkills or "")
        if hasattr(offer, 'domain'):
            offer_text += " " + (offer.domain or "")
        ...
        if hasattr(student, 'major') and student.major:
            ...
            if major_lower in offer_text:
                major_relevance = 1.0
            elif 'computer' in major_lower or 'software' in major_lower:
                if any(tech in offer_text for tech in ['developer', 'programming', 'coding']):
                    major_relevance = 0.95
            elif 'data' in major_lower or 'science' in major_lower:
                if any(tech in offer_text for tech in ['data', 'analytics', 'ml', 'ai']):
                    major_relevance = 0.95
        
        return level_score * major_relevance
    
    def calculate_experience_match(self, student, offer=None) -> float:
        ...
        if hasattr(student, 'projects') and student.projects:
            relevant_projects = 0
            if offer:
                offer_text = f"{offer.title} {offer.description} {offer.requiredSkills or ''} {offer.domain or ''}".lower()
                for project in student.projects[:5]:
                    project_lower = project.lower()
                    if any(tech in project_lower for tech in offer_text.split()[:20]):
                        relevant_projects += 1
    ...
    def calculate_location_match(self, student, offer) -> float:
        """Calculate location match"""
        return 0.9
    
    def calculate_projects_match(self, student, offer) -> float:
    ...
    def calculate_certifications_match(self, student, offer) -> float:
        ...
        for cert in student.certifications[:3]:
            ...
            if any(tech in offer_text for tech in cert_text.split()[:5]):
                relevance_score += 0.35
        
        return min(relevance_score, 1.0)
    
    def calculate_overall_score(self, student, offer) -> Dict:
        """Calculate overall compatibility score with all factors"""
        student_skills = self.extract_skills_from_student(student)
        offer_skills = self.extract_skills_from_offer(offer)
        
        skills_score = self.calculate_skills_match(student_skills, offer_skills)
        education_score = self.calculate_education_match(student, offer)
        experience_score = self.calculate_experience_match(student, offer)
        location_score = self.calculate_location_match(student, offer)
        projects_score = self.calculate_projects_match(student, offer)
        certifications_score = self.calculate_certifications_match(student, offer)
    ...
    def recommend_offers_for_student(self, student, offers: List, limit: int = 10) -> List:
        """Recommend offers for a student with full details"""
        recommendations = []
        
        for offer in offers:
            score_data = self.calculate_overall_score(student, offer)
            if score_data["match_percentage"] >= Config.MIN_MATCH_SCORE:
                recommendations.append({
                    ...
                    # Add offer details
                    "title": offer.title,
                    "description": offer.description[:200] + "..." if offer.description and len(offer.description) > 200 else offer.description,
                    ...
                    # Add company details
                    "company_id": offer.company.id if hasattr(offer, 'company') else None,
                    "company_name": offer.company.name if hasattr(offer, 'company') else "Unknown Company",
                    "company_city": offer.company.city if hasattr(offer, 'company') else None,
                    "company_country": offer.company.country if hasattr(offer, 'company') else None
                })
    ...
    def recommend_students_for_company(self, company, students: List, limit: int = 10) -> List:
        ...
        for student in students:
            # Create a proper offer-like object with required attributes
            class CompanyOffer:
            ...
            temp_offer = CompanyOffer(company)
            score_data = self.calculate_overall_score(student, temp_offer)
