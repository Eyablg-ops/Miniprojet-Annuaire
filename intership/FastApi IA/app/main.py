...
async def fetch_all_offers():
    """Fetch all offers from Spring Boot API"""
    try:
        url = f"{Config.SPRING_BOOT_API_URL}/recommendation/offers"
        ...
        if response.status_code == 200:
            data = response.json()
            logger.info(f"Fetched {len(data)} offers")
            # Log first offer to see structure
            if data:
                logger.info(f"First offer: {data[0].get('title', 'Unknown')}")
            return data
        else:
...
@app.post("/recommend/student-to-offers")
async def recommend_offers_for_student(request: MatchRequest):
    """Recommend offers for a student"""
    try:
        ...
        offers_data = await fetch_all_offers()
        if not offers_data:
            return {
        ...
        offers = [Offer(**off) for off in offers_data]
        
        recommendations = matching_engine.recommend_offers_for_student(
            student, offers, request.limit
        )
        ...
        logger.error(f"Error in recommend_offers: {e}", exc_info=True)
